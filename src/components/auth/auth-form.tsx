...
export function AuthForm({ redirectTo = "/" }: AuthFormProps) {
  ...
  if (!SUPABASE_ENABLED) {
    return (
      <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5 text-center">
        <p className="font-semibold text-amber-500">{t("devModeTitle")}</p>
        <p className="mt-1 text-sm text-muted-foreground">{t("devModeDesc")}</p>
      </div>
    )
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Successful — the middleware will redirect or the auth listener will update state
  ...
  return (
    <div className="flex w-full flex-col gap-5">
      ...
      <Button
        type="button"
        variant="outline"
        className="h-11 w-full gap-2.5 rounded-xl border-border/60"
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
        ...
        <div className="relative">
          ...
          <Input
            ...
            required
            disabled={loading}
            className={cn("h-11 rounded-xl ps-9", "pe-10")}
          />
          <button
            type="button"
